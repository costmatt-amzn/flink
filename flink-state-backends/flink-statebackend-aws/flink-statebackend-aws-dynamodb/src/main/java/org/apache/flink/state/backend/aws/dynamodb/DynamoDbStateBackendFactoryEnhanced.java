...
/**
 * Factory for creating {@link DynamoDbStateBackendEnhanced} instances.
 *
 * <p>This factory implements the Flink {@link StateBackendFactory} interface to allow
 * the DynamoDB Enhanced state backend to be instantiated from configuration.
 *
 ...
 * <pre>{@code
 * state.backend: dynamodb-enhanced
 * }</pre>
 */
public class DynamoDbStateBackendFactoryEnhanced extends AbstractAwsStateBackendFactory<DynamoDbStateBackendEnhanced, DynamoDbClient, TableDescription> {
    ...
    protected void validateConfiguration(final Configuration configuration) throws IllegalArgumentException {
        ...
        if (configuration.get(DynamoDbStateBackendOptions.TABLE_NAME) == null) {
            throw new IllegalArgumentException(
                    "Missing required configuration for DynamoDB Enhanced state backend. " +
                            "Please specify " + DynamoDbStateBackendOptions.TABLE_NAME.key());
        }
    }

    @Override
    protected DynamoDbStateBackendEnhanced createStateBackend(final Configuration configuration) {
        return new DynamoDbStateBackendEnhanced(configuration);
    }

    @Override
    public String toString() {
        return "DynamoDbEnhancedStateBackendFactory";
    }
}
